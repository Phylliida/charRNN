...
def rnn_minibatch_sequencer(raw_data, batch_size, sequence_size, nb_epochs, startBatch=0, startEpoch=0):
    """
	...
	:param startBatch: which batch to start on (default is 0)
	:param startEpoch: which epoch to start on (default is 0), will run until we finish epoch (startEpoch+nb_epochs-1)
    :return:
    ...
    # modified so we can resume
    isFirstLoop = True
    for epoch in range(startEpoch, startEpoch+nb_epochs):
        if isFirstLoop:
            batches = range(startBatch, nb_batches)
            isFirstLoop = False
        else:
            batches = range(nb_batches)
...
def print_learning_learned_comparison(X, Y, losses, bookranges, batch_loss, batch_accuracy, epoch_size, index, epoch, totalEpochsDoing, verbosity=2):
    ...
    for k in range(batch_size):
        ...
        if verbosity >= 2: print(print_string.format(decx, decy, loss_string))
        index += sequence_len
    ...
    if verbosity >= 2: print(footer)
    # print statistics
    batch_index = start_index_in_epoch // (batch_size * sequence_len)
    batch_string = "batch {}/{} in epoch {}/{},".format(batch_index, epoch_size, epoch, totalEpochsDoing)
